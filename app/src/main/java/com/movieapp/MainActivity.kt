...
class MainActivity : AppCompatActivity(),FragmentCommunicator {
    ...
    private fun getSaveFavorites() { // load from SharedPreferences saved favorites
        ...
        if (appSharedPrefs!!.contains("file")) {
            ...
            for(i in 0 until listCategoriesFavorites.size){       // add to live data
    ...
    private fun setFavoritesBehavior(selectedCategory: Category) {
        ...
        if (listCategoriesFavorites.isNotEmpty()) {    // check if have favorites
        ...
        } else {                // fake request to clear the favorites
    ...
    private fun startFragment(position: Int) {
        ...
        transaction.setCustomAnimations(   // set fragment load animation
            R.anim.enter,
            R.anim.exit,
            R.anim.pop_enter,
            R.anim.pop_exit
        ...
        val selectedMovie = moviesListVM!!.getMovies().value?.get(position)!!  // get specific movie by position

        val newDetailsFragment: DetailsFragment = DetailsFragment.newInstance(  // set the the parameter to the fragment
            selectedMovie.id,
        ...
        newDetailsFragment.setCommunicator(this)  // set listener to detect when fragment is detached
    ...
    fun ObserverOfChange() {
        moviesListVM!!.getMovies().observe(this, object : Observer<ArrayList<MovieModel?>> { // movies list live data observing
    ...
    // when fragment detached on favorites tab updated the movies list
