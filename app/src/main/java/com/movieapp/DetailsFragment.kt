...
class DetailsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        setVisibelViews() // set views

        requestFavorites() // update live data

        ObserverOfChange()

        return view
    }
 // make live data request
    ...
    private fun setVisibelViews() {
        ...
        Glide.with(requireContext())            // set glide to loading movie's image
            .load(url)
            .signature(IntegerVersionSignature(versionNumber))  // set daily clean catch
            ...
            .into(binding!!.detailsImg) // add to image

        binding!!.favoriteImg.setOnClickListener {  // set favorite slected
            if (!isSelected) {          // if not in favorite add to favorite
                isSelected = true            // selected flag (in favorite)
                moviesListVM!!.addToFavorites(category!!, page!!, movieId.toString())        // live data request add to favorite
                binding!!.favoriteImg.setImageResource(R.drawable.ic_baseline_star_50)       // set the image

            }else{       //  if in favorite remove from favorite
                moviesListVM!!.removeFavorites( movieId.toString())            // live data request remove from favorite
                binding!!.favoriteImg.setImageResource(R.drawable.ic_baseline_star_border_50)
                isSelected = false           // selected flag (not in favorite)
            }
        }
    }
    ...
    override fun onDetach() {  // fragment closed
        super.onDetach()
        communicator.fragmentDetached()     //  fragment closed function
    }
    ...
    fun ObserverOfChange() {
        moviesListVM!!.getFavorites()       // favorites live data observing
            .observe(viewLifecycleOwner, object : Observer<HashMap<String, FavoriteModel>> {
                override fun onChanged(map: HashMap<String, FavoriteModel>?) {
                    ...
                    if (map != null && map.containsKey(movieId.toString())){  // check if on live data in favorite
                        isSelected = true
                        binding!!.favoriteImg.setImageResource(R.drawable.ic_baseline_star_50)
                    }

                    val parserJsonFavorites = parserJsonFavorites(map)                   // set to json string
                    ...
                    personalSharedPrefsEditor?.putString(
                        "file",
                        parserJsonFavorites                              // save to SharedPreferences
                    )
                    personalSharedPrefsEditor?.apply()
                    fromJsonFavorites(appSharedPrefs.getString("file", "")!!)       // save data after observing
                }
            }
            )
    }
    // set the json string to data
